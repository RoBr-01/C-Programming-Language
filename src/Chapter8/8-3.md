...
>***Note***
>
>That spelling is correct, it is `creat` and not `create`. The UNIX tradition aims to keep function names as short as possible (and yes `unlink` is an odd one out...)

`open` is rather like the `fopen` discussed in [Chapter 7](../Chapter7/7-0.md), except that instead of returning a file pointer, it returns a file descriptor, which is just an int. `open` returns -1 if any error occurs.
...
This program creates the output file with fixed permissions of `0666`. With the `stat` system call, described in [Section 8.6](../Chapter8/8-6.md), we can determine the mode of an existing file and thus give the same mode to the copy.
...
[**Exercise 8-1.**](../Solutions/Chapter8/E8-1.md) Rewrite the program `cat` from [Chapter 7](../Chapter7/7-0.md) using `read`, `write`, `open`, and `close` instead of their standard library equivalents. Perform experiments to determine the relative speeds of the two versions.
